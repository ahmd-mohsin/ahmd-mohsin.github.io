        ...
        <div>
          {experiences.map((exp, i) => (
            <motion.div
              ...
              className="border-t border-[#d4d4d8]/20 py-8 grid md:grid-cols-[200px_1fr] gap-6 md:gap-12"
              data-testid={`row-experience-${i}`}
            >
              <div className="shrink-0">
                <p className="text-sm font-semibold text-[#8C1515] leading-snug mb-2">
                  {exp.org}
                </p>
                <p className="font-mono text-[10px] text-[#9a9a9a] leading-relaxed">
                  {exp.dateRange}
                </p>
                <p className="font-mono text-[10px] text-[#9a9a9a] mt-1">
                  {exp.advisor}
                </p>
              </div>
              ...
              <div className="space-y-5">
                {exp.projects.map((project, j) => (
                  <div key={j}>
                    <p className="text-sm text-[#8C1515] font-medium mb-2">
                      {project.name}
                    </p>
                    <ul className="space-y-2">
                      {project.bullets.map((bullet, k) => (
                        <li key={k} className="flex gap-3 text-sm leading-relaxed text-[#262626]">
                          <span className="text-[#9a9a9a] shrink-0 mt-0.5">—</span>
                          <span>{bullet}</span>
                        </li>
                      ))}
                    </ul>
