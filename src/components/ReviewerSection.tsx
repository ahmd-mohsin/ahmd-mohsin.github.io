...
function Tag({ children }: { children: ReactNode }) {
  return (
    <span className="border border-white/[0.08] px-3 py-1.5 font-mono text-[11px] text-white/30 hover:text-white/50 hover:border-white/15 transition-colors">
      {children}
    </span>
  );
}
      ...
      <div className="space-y-0">
          ...
          <SubSection label="Journal Reviewer">
            <div className="flex flex-wrap gap-2">
              {journals.map((j, i) => (
                <span
                  key={j.name}
                  className="border border-white/[0.08] px-3 py-1.5 font-mono text-[11px] text-white/30"
                  data-testid={`tag-journal-${i}`}
                >
                  {j.name}
                  <span className="text-white/15 mx-1.5">/</span>
                  <span className="text-white/20">{j.desc}</span>
                </span>
              ))}
            </div>
          </SubSection>
        ...
        >
          <SubSection label="Technical Program Committee">
            ...
            <p className="font-mono text-[10px] uppercase tracking-[0.25em] text-white/20 mb-4">
              Leadership
            </p>
            <div className="flex flex-wrap gap-2">
              {leadership.map((item, i) => (
                <span
                  key={i}
                  className="border border-white/[0.08] px-3 py-1.5 font-mono text-[11px]"
                >
                  <span className="text-white/50">{item.role}</span>
                  <span className="text-white/15 mx-1.5">—</span>
                  <span className="text-white/25">{item.venue}</span>
                </span>
              ))}
            </div>
          </SubSection>
        ...
        >
          <SubSection label="Awards & Recognitions">
            <div className="space-y-2.5 mb-8">
              {awards.map((award, i) => (
                <p key={i} className="flex gap-3 text-sm text-white/35">
