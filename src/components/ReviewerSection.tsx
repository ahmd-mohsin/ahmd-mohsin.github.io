...
function Tag({ children }: { children: ReactNode }) {
  return (
    <span className="border border-[#d4d4d8]/25 bg-[#f4f4f5]/60 px-3 py-1.5 font-mono text-[11px] text-[#4b5563] hover:text-[#8C1515] hover:border-[#8C1515]/40 transition-colors">
      {children}
    </span>
  );
}
...
function SubSection({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div className="border-t border-[#d4d4d8]/20 pt-8">
      <p className="font-mono text-[10px] uppercase tracking-[0.25em] text-[#8C1515] mb-5">
        {label}
      </p>
      {children}
    </div>
      ...
      <GlassPanel className="p-8 md:p-10">
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.05 }}>
          <SubSection label="Journal Reviewer">
            <div className="flex flex-wrap gap-2">
              {journals.map((j, i) => (
                <span key={j.name} className="border border-[#d4d4d8]/25 bg-[#f4f4f5]/60 px-3 py-1.5 font-mono text-[11px] text-[#6f1010]" data-testid={`tag-journal-${i}`}>
                  {j.name}
                  <span className="text-[#9a9a9a] mx-1.5">/</span>
                  <span className="text-[#4b5563]">{j.desc}</span>
                </span>
              ))}
            </div>
          </SubSection>
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }}>
          <SubSection label="Technical Program Committee">
            ...
            <p className="font-mono text-[10px] uppercase tracking-[0.25em] text-[#8C1515] mb-4">Leadership</p>
            <div className="flex flex-wrap gap-2">
              {leadership.map((item, i) => (
                <span key={i} className="border border-[#d4d4d8]/25 bg-[#f4f4f5]/60 px-3 py-1.5 font-mono text-[11px]">
                  <span className="text-[#6f1010] font-medium">{item.role}</span>
                  <span className="text-[#9a9a9a] mx-1.5">—</span>
                  <span className="text-[#4b5563]">{item.venue}</span>
                </span>
              ))}
            </div>
          </SubSection>
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.15 }}>
          <SubSection label="Awards & Recognitions">
            <div className="space-y-2.5 mb-8">
              {awards.map((award, i) => (
                <p key={i} className="flex gap-3 text-sm text-[#262626]">
                  <span className="text-[#9a9a9a] shrink-0">—</span>
                  {award}
                </p>
              ))}
            </div>
            <p className="font-mono text-[10px] uppercase tracking-[0.25em] text-[#8C1515] mb-4">Travel Grants</p>
            <div className="flex flex-wrap gap-2">
